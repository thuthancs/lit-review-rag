"""
Streamlit app for Literature Review RAG Assistant (Simple Offline Version)
Uses file-based storage to completely bypass Weaviate and OpenAI.
Run with: streamlit run app_simple.py
"""

import streamlit as st
from pdf_processor import PDFProcessor
from rag_queries_simple import (
    get_simple_rag_manager,
    generate_gap_analysis_simple,
    chat_with_papers_simple,
)
import tempfile
import os
...
# Page config
st.set_page_config(
    page_title="Literature Review Assistant (Offline Mode)",
    page_icon="📚",
    layout="wide",
...
def process_uploaded_papers(uploaded_files, manual_title, manual_authors, manual_year):
    """Process and upload papers to simple storage"""
    progress_bar = st.sidebar.progress(0)
    status_text = st.sidebar.empty()

    processor = PDFProcessor()
    rag_manager = get_simple_rag_manager()
    ...
    if successful > 0:
        ...
        st.sidebar.info("💾 Stored locally (no external dependencies)")
...
def display_collection_stats():
    ...
    try:
        ...
        papers = rag_manager.get_all_papers()

        st.sidebar.markdown("---")
        st.sidebar.markdown("### 📊 Collection Stats (Offline)")
            ...
            with st.sidebar.expander("📚 View All Papers"):
                for paper in papers[:5]:  # Show first 5
                    ...
                    <div class="paper-card">
                        ...
                        <small>{paper.get('authors', ['Unknown'])[0] if paper.get('authors') else 'Unknown'} ({paper.get('publication_year', 'N/A')})</small><br/>
                        <small>Chunks: {paper.get('chunk_count', 0)}</small>
                    </div>
                    """,
                        unsafe_allow_html=True,
                    )
...
def gap_analysis_tab():
    ...
    st.markdown("### 🔍 Research Gap Analysis (Offline Mode)")
    st.markdown("*Basic text-based analysis using local storage*")
    
    # Show offline mode notice
    st.markdown(
        """
        <div class="offline-warning">
            <strong>✅ Offline Mode:</strong> This analysis uses local file storage and basic text matching. 
            No external APIs or services required - everything runs locally on your machine.
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    if st.button("🔍 Analyze Research Gaps", type="primary", use_container_width=True):
        with st.spinner("🤔 Analyzing papers for research gaps (Offline Mode)..."):
            try:
                result = generate_gap_analysis_simple(focus_area if focus_area else "research")
                if result["success"]:
    ...
    if st.session_state.last_gap_analysis:
        ...
        with col1:
            st.download_button(
                ...
                data=analysis["gaps"],
                file_name=f"research_gaps_offline_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
                mime="text/plain",
...
def chat_tab():
    ...
    st.markdown("### 💬 Chat with Your Papers (Offline Mode)")
    st.markdown("*Basic text-matching responses using local storage*")
    
    # Show offline mode notice
    st.markdown(
        """
        <div class="offline-warning">
            <strong>✅ Offline Mode:</strong> This chat uses local file storage and basic text matching. 
            No external APIs or services required - everything runs locally on your machine.
        </div>
        """,
        unsafe_allow_html=True
        ...
        for i, msg in enumerate(st.session_state.chat_history):
            if msg["role"] == "user":
                ...
                <div class="chat-message assistant-message">
                    <strong>Assistant (Offline Mode):</strong><br/>
                    {msg['content']}
                </div>
                """,
                    unsafe_allow_html=True,
    ...
    if st.button("📤 Send Question", type="primary", use_container_width=True):
        ...
        # Get answer
        with st.spinner("🔍 Searching papers and generating answer (Offline Mode)..."):
            try:
                result = chat_with_papers_simple(question)
                if result["success"]:
...
def main():
    ...
    # Header
    st.markdown(
        '<p class="main-header">📚 Literature Review Assistant (Offline Mode)</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<p class="sub-header">Completely offline RAG system using local file storage (no external dependencies)</p>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        """
        <div class="offline-warning">
            <strong>✅ Offline Mode Active:</strong> This version uses local file storage and basic text matching. 
            No external APIs, no Weaviate, no OpenAI - everything runs locally on your machine.
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    # Sidebar
    with st.sidebar:
        st.markdown("## 🎛️ Control Panel (Offline Mode)")

        # Always show as connected since we're using local storage
        st.markdown("### 🔌 Connection Status")
        st.success("✓ Connected to Local Storage")
        ...
        st.markdown(
            """
        <small>
        Built with Local File Storage<br/>
    ...
    st.markdown("### 🚀 Quick Start Guide (Offline Mode)")
    ...
    # Main tabs
    tab1, tab2 = st.tabs(["🔍 Gap Analysis (Offline)", "💬 Chat with Papers (Offline)"])
